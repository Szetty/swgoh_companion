defmodule SWGOHCompanionWeb.UserAuth do
  import Plug.Conn
  import Phoenix.Controller

  alias Phoenix.LiveView
  alias SWGOHCompanionWeb.Live
  alias SWGOHCompanionWeb.Router.Helpers, as: Routes

  def on_mount(:current_user, _params, session, socket) do
    case session do
      %{"user_id" => user_id} ->
        {:cont, LiveView.assign_new(socket, :current_user, fn -> user_id end)}
  ...
  def on_mount(:ensure_authenticated, _params, session, socket) do
    case session do
      %{"user_id" => user_id} ->
        new_socket = LiveView.assign_new(socket, :current_user, fn -> user_id end)
  ...
  def log_out_user(conn) do
    if live_socket_id = get_session(conn, :live_socket_id) do
      SWGOHCompanionWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def signed_in_path(conn) do
    Live.Helpers.home_path(conn.assigns.current_user)
  end
end
